...
int main(){
    ...
    */
	...
	OPTIMAL_ROUTE=(int*)malloc((N+1)*sizeof(int));
        ...
        cudaSetDevice(0);
        printf("\n greedy \n");
        int BEST_GLOBAL_SOLUTION=rutainicial(OPTIMAL_ROUTE,NODE_COORDINATE_2D,NEW_LIST_GLOBAL,NEW_LIST_INDX_GLOBAL,NN_LIST_cl,POS_IN_ROUTE_OP);
	...
	float entropy=log2(M);
        for (it=0;it<ITERACION;it++){
            ...
            for (i=0;i<N_GPU;i++){
                ...
                d_SUCCESSOR_ROUTE_OP_MGPU[i],d_POS_IN_ROUTE_OP[i],d_OPTIMAL_ROUTE_MGPU[i],8,d_state[i],d_NN_LIST_CL_MGPU[i],d_NEW_LIST[i],d_NEW_LIST_INDX[i],d_RANDOM_DEBUBG[i],s_s_flag);
	    ...
	    double end_1 =omp_get_wtime();
            ...
            */
		    ...
		    BEST_GLOBAL_SOLUTION=GLOBAL_COST[0];
                    ...
                    SAVE_LAST_IMRPOVED(LAST_IMPROVE_ITERATION,BEST_GLOBAL_SOLUTION,x); 
		    //printf("\n tau max =%.16lf \n tau min =%.16lf \n",tau_max,tau_min);
                    for (i=0;i<N+1;i++){
	    ...
	    mean_entropy_Pheromone=shannon_entropy_pheromone(HEURISTIC_PHEROMONE,PROB_MATRIX,ENTROPY_VECTOR_PHEROMONE,1);           
            HEURISTIC_PHEROMONE_CALCULATION<<<N,cl>>>(d_NODE_COORDINATE_2D,d_PHEROMONE_MATRIX,
            d_HEURISTIC_PHEROMONE,d_NN_LIST_cl,alpha,beta);
            cudaMemcpy(HEURISTIC_PHEROMONE,d_HEURISTIC_PHEROMONE,(N*c_l)*sizeof(float),cudaMemcpyDeviceToHost);
	    mean_entropy_Heuristic=shannon_entropy_pheromone(HEURISTIC_PHEROMONE,PROB_MATRIX,ENTROPY_VECTOR_PHEROMONE_H,1);           
