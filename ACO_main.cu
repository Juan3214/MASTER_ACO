...
int main(){
    ...
    */
	...
	float tau_min,tau_max;
        ...
        int *d_COST_MGPU[N_GPU];int *d_NN_LIST_CL_MGPU[N_GPU]; 
        float *d_NODE_COORDINATE_MGPU[N_GPU];  
        int *d_OPTIMAL_ROUTE_MGPU[N_GPU];
        int *d_NEW_LIST[N_GPU],*d_NEW_LIST_INDX[N_GPU],*d_POS_IN_ROUTE_MGPU[N_GPU],*d_ROUTE_AUX[N_GPU];
        float *d_HEURISTIC_PHEROMONE_MGPU[N_GPU];
	int *d_POS_IN_ROUTE_OP[N_GPU];
        ...
        int *d_BEST_ANT,*d_OPTIMAL_ROUTE,*d_GLOBAL_COST,*d_GLOBAL_NEW_LIST; 
        cudaSetDevice(i_GPU);
        float *d_HEURISTIC_PHEROMONE,*d_PHEROMONE_MATRIX; 
        float *HEURISTIC_PHEROMONE;
        bool *VISITED_LIST;
        int *OPTIMAL_ROUTE,*GLOBAL_COST,*BEST_ANT;
        float *d_NODE_COORDINATE_2D;
        curandState *d_state[N_GPU];
        cudaMalloc( (void **) &d_BEST_ANT, N_GPU*M*sizeof(int) );
        cudaMalloc( (void **) &d_OPTIMAL_ROUTE, (N+1)*sizeof(int) );
        cudaMalloc( (void **) &d_HEURISTIC_PHEROMONE, N*cl*sizeof(float));
        cudaMalloc( (void **) &d_PHEROMONE_MATRIX, N*cl*sizeof(float) );
        cudaMalloc( (void **) &d_GLOBAL_COST, N_GPU*M*sizeof( int ) );
        cudaMalloc( (void **) &d_GLOBAL_NEW_LIST, N_GPU*(N+1)*M*sizeof( int ) );
        cudaMalloc( (void **) &d_NODE_COORDINATE_2D, N*2*sizeof(float ) );
	...
	OPTIMAL_ROUTE=(int*)malloc((N+1)*sizeof(int));
        ...
        for (i=0;i<N_GPU;i++){
            ...
            cudaMalloc( (void **) &d_NODE_COORDINATE_MGPU[i], 2*N*sizeof( float ) );
            cudaMemcpy(d_NODE_COORDINATE_MGPU[i],NODE_COORDINATE_2D,2*N*sizeof( float ),cudaMemcpyHostToDevice);
            cudaMalloc( (void **) &d_NEW_LIST[i], M*(N+1)*sizeof( int ) );
            cudaMalloc( (void **) &d_POS_IN_ROUTE_MGPU[i], M*N*sizeof( int ) );
            cudaMalloc( (void **) &d_ROUTE_AUX[i], M*(N+1)*sizeof( int ) );
            cudaMalloc( (void **) &d_NEW_LIST_INDX[i], M*(N+1)*sizeof( int ) );
    	    cudaMalloc( (void **) &d_RANDOM_DEBUBG[i], M*N*sizeof(int) );
            cudaMalloc( (void **) &d_COST_MGPU[i], M*sizeof( int ) );
            cudaMalloc( (void **) &d_LOCAL_SEARCH_LIST_MGPU[i], M*(N+1)*sizeof( int ) );
            cudaMalloc( (void **) &d_NN_LIST_CL_MGPU[i], cl*N*sizeof( int ) ); 
            cudaMemcpy(d_NN_LIST_CL_MGPU[i],NN_LIST_cl,(c_l*N)*sizeof(int),cudaMemcpyHostToDevice);
            cudaMalloc( (void **) &d_HEURISTIC_PHEROMONE_MGPU[i], N*cl*sizeof( float ) );
            cudaMalloc( (void **) &d_POS_IN_ROUTE_OP[i], N*sizeof( int ) );
            cudaMalloc( (void **) &d_OPTIMAL_ROUTE_MGPU[i], N*sizeof( int ) );
	...
	if ( 1 == ACO_ALG)PHEROMONE_CHECK_MMAS<<<((N*cl+32-(N*cl%32)))/32,32>>>(d_PHEROMONE_MATRIX, tau_max, tau_min);;
        ...
        thrust::device_ptr<int> dev_inx = thrust::device_pointer_cast(d_BEST_ANT); 
	...
	float mean_entropy_Pheromone=0.0;
        for (it=0;it<ITERACION;it++){
            ...
            for (i=0;i<N_GPU;i++){
                ...
                gpuErrchk(cudaMemcpyAsync(GLOBAL_COST+i*M,d_COST_MGPU[i],M*sizeof(int),cudaMemcpyDeviceToHost));   
                gpuErrchk(cudaMemcpyAsync(NEW_LIST_GLOBAL+i*M*(N+1),d_NEW_LIST[i],(N+1)*M*sizeof(int),cudaMemcpyDeviceToHost));
                gpuErrchk(cudaMemcpyAsync(NEW_LIST_INDX_GLOBAL+i*M*(N+1),d_NEW_LIST_INDX[i],(N+1)*M*sizeof(int),cudaMemcpyDeviceToHost));   
                gpuErrchk(cudaMemcpyAsync(RANDOM_DEBUG+i*M*N,d_RANDOM_DEBUBG[i],N*M*sizeof(int),cudaMemcpyDeviceToHost));   
                cudaDeviceSynchronize();
		...
		printf("%d best best\n",BEST_GLOBAL_SOLUTION);
		    ...
		    BEST_GLOBAL_SOLUTION=GLOBAL_COST[0];
                    ...
                    printf("\n best solution = %d in iteration = %d on experiment %d using alpha= %f and beta=%f\n",BEST_GLOBAL_SOLUTION,LAST_IMPROVE_ITERATION,x,alpha,beta);
